...
// const defaultLocations = users.value.reduce((acc : any, user : User) => {
...
// const defaultStatuses = users.value.reduce((acc : any, user : User) => {
...
<template>
  <UDashboardPage>
    <UDashboardPanel grow>
      <UDashboardNavbar title="Utilisateurs·trices" :badge="users.length">
        <template #right>
      ...
      <UTable :columns="columns" :rows="users" :loading="pending" v-model="selected" v-model:sort="sort"
        sort-mode="manual" class="w-full" :ui="{ divide: 'divide-gray-200 dark:divide-gray-800' }" @select="onSelect">

        <template #speudo-data="{ row }">
          <div class="flex items-center gap-3">
            <UAvatar v-bind="row.avatar" :alt="row.speudo" size="xs" />

            <span class="text-gray-900 dark:text-white font-medium">{{ row.speudo }}</span>
          </div>
        </template>
